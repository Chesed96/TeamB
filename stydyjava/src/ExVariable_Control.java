...
public class ExVariable_Control {
	public static void main(String[] args) {
		//변수~제어문 복습.
		//변수 : 데이터를 담기 위한 그릇, 메모리의 주소부를 개발자가 직접 참조하는 것이 아니라 변수의 이름을 가지고 접근함
		//변수의타입 변수의이름; ←선언
		int number;	//초기화되지 않은 변수는 사용이 불가능하다.
		number = 10;	//변수의 선언과 변수에 값을 할당하면 초기화 라고 한다.(사용할 준비 완료)
		System.out.println(number);
		//double ← 실수형 데이터
		//boolean ← 부울형 true, false 논리형 데이터
		//★ 문자열 데이터 → 숫자형 테이터
		//	숫자형 데이터 → 문자열 데이터
		String str = "10";
		...
		char chr = '1';
		//num이라는 변수에 재할당 해보기
		num = Integer.parseInt(chr +"");	//어떤 일반 변수를 String 으로 바꾸는 방법 : 변수+String
		...
		//if(조건식){ 조건식이 만족했을 때 실행되는 부분 } else { 조건식을 만족하지 못했을 때 실행되는 부분 }
		if(1>0 && 0>0) {// 1*0 = 0;
		...
		//문제) Scannaer로 어떤 값을 입력받는다. 그리고 그 값이 A와 같다면 A입니다. 를 출력하고
		//A와 같지 않다면 A가 아닙니다.를 출력
